...
var qbws,
    ...
    username = 'Admin',
    password = 'Password#1984',
    /// companyFile = 'C:\\Users\\admin\\Desktop\\QBD Materials\\Sample QB Files\\sample_product-based business.QBW',
    companyFile = 'C:\\Users\\admin\\Desktop\\QBD Materials\\Sample QB Files\\GoldMiner.QBW',
    req = [],
    config = {
        'verbosity': 2
    };

// defining wsdl "services" with "port" 
qbws = {
    QBWebConnectorSvc: {
...
function buildWsdl(wsdlFileLocation) {
    ...
    // console.log("WSDL : " + wsdl); -- prints the Whole WSDL file 
...
function buildRequest() {
    ...
    // VendorQuery Fetch Request with Max Return Resultset size of 1
    inputXMLDoc = builder.create('QBXML', { version: '1.0' })
        .instruction('qbxml', 'version="4.0"')
        .ele('QBXMLMsgsRq', { 'onError': 'stopOnError' })
        .ele('VendorQueryRq', { 'requestID': '0' }) // CustomerQueryRq
        .ele('MaxReturned')
        .text('1');

    strRequestXML = inputXMLDoc.end({ 'pretty': true });

    console.log("VendorQueryRq Query XML : \n" + strRequestXML);
    request.push(strRequestXML);

    // strRequestXML = "";
    // inputXMLDoc = null;

    // // building up the fetch Customer query based on "name" search.. 
    // inputXMLDoc = builder.create('QBXML', { version: '1.0' })
    //     .instruction('qbxml', 'version="13.0"')
    //     .ele('QBXMLMsgsRq', { 'onError': 'stopOnError' })
    //     .com("Customer Query Request")
    //     .ele('CustomerQueryRq', { 'requestID': '1' })
    //         .ele('MaxReturned').text('1').up() // returns top level.. 
    //         .ele('ActiveStatus', 'ActiveOnly').up()
    //         .ele("NameFilter")
    //             .ele("MatchCriterion", "Contains").up()
    //             .ele("Name", "Mickey").up()

    // strRequestXML = inputXMLDoc.end({ 'pretty': true });
    // console.log("Customer Query Request XML format :\n " + strRequestXML);
    // request.push(strRequestXML);

    // building up the insert New Customer Request
    inputXMLDoc = builder.create('QBXML', { version: '1.0' })
        .instruction('qbxml', 'version="4.0"')
        .ele('QBXMLMsgsRq', { 'onError': 'stopOnError' })
        .ele("CustomerAddRq", { 'requestID': '1' })
        .ele("CustomerAdd")
        .ele("Name", "Mickey Mouse").up()
        .ele("IsActive", 1).up()
        .ele("CompanyName", "DisneyLand").up()
        .ele("Salutation", "Mr").up()
        .ele("FirstName", "Mickey").up()
        .ele("LastName", "Mouse").up()
        //.ele("JobTitle", "Cartoon").up()
        .ele("Email", "mickey.mouse@disneyland.com").up()

    strRequestXML = inputXMLDoc.end({ 'pretty': true });
    console.log("New Customer Add Request : \n" + strRequestXML);
    request.push(strRequestXML);
    ...
    //             + "<CompanyName>Disney</CompanyName>"
    //             + "<Salutation>Mr.</Salutation>"
    //             + "<FirstName>Mickey</FirstName>"
    //             + "<LastName>Mouse</LastName>"
    //             + "<JobTitle>Cartoon</JobTitle>"
    //             + "<BillAddress>"
    //             + "<Addr1>Disneyland</Addr1>"
    //             + "<Addr2>Hongkong</Addr2>"
    //             + "</BillAddress>"
    //             + "<Email>mickey.mouse@somwhere.com</Email>"
    //         + "</CustomerAdd>"
    //     + "</CustomerAddRq>"
    //     + "</QBXMLMsgsRq>"
    //     + "</QBXML>");
...
function announceMethod(name, params) {
    ...
    if (config.verbosity > 0) {
        serviceLog(chalk.green('WebMethod: ' + name + '() has been called by QBWebConnector'));
    }

    if (config.verbosity > 1) {
        if (objectNotEmpty(params)) {
            serviceLog('    Request Parameters :');
            for (arg in params) {
                if (params.hasOwnProperty(arg)) {
                    ...
                    // TODO: DRY this up
                    if (argType === 'object') {
                        serviceLog('        ' + argType + ' ' + arg + ' = ' + JSON.stringify(params[arg], null, 2));
                    } else {
            ...
            serviceLog('    No Request parameters.');
        }
    }
}
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate =
    function (args) {
        ...
        authReturn[0] = uuid.v1(); // Ticket Token  after successful authentication
        ...
        if (args.strUserName.trim() === username && args.strPassword.trim() === password) {
            req = buildRequest(); // Building up the main request here
            ...
            if (req.length === 0) {
                ...
                authReturn[1] = companyFile; // constant defined at global level
            }
        } else {
            authReturn[1] = 'nvu';
        }
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.clientVersion =
    function (args) {
        ...
        console.log("Parsed Version : " + strVersion);
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.receiveResponseXML =
    function (args) {
        ...
        if (objectNotEmpty(hresult)) {
            ...
            serviceLog('    Counter : ' + counter + ',  Percentage : ' + percentage + ", Request length : " + req.length);
            if (percentage >= 100) {
                counter = 0;
            }

            // QVWC throws an error if the return value contains a decimal
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.sendRequestXML =
    function (args) {
        ...
        if (counter < total) {
            request = req[counter];
            args['strHCPResponse'] = request; // experiment
            announceMethod('sendRequestXML', args);
            // serviceLog('    XML Request : ' + request);
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.serverVersion =
    function (args) {
        ...
        serviceLog('    No Response parameters.');
...
// defining web service for GetTutorialName
// qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.GetTutorialName = function (args) {
//     announceMethod('GetTutorialName', args);

//     return {
//         TutorialName: { 'string': 'Teaching WSDL Basics' }
//     };
// };
...
module.exports.run = function runQBWS() {
    ...
    // creates SOAP Server that start listening on server with provides services
